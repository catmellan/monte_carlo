...
with st.sidebar:
    ...
    init_balance = st.number_input("💰 Начальный баланс", value=1000.0, help="Стартовая сумма в каждой симуляции")
    winrate = st.slider("🎯 Winrate (%)", 0, 100, 50, help="Процент выигрыша по стратегии")
    rr = st.number_input("⚖️ Reward/Risk (RR)", value=1.0, help="Соотношение прибыли к риску в одной сделке")
    risk_pct = st.number_input("🔥 Риск на сделку (%)", value=15.0, help="Сколько % капитала рискуется в каждой сделке")
    num_trades = st.number_input("🔁 Сделок в симуляции", min_value=1, max_value=1000, value=100, help="Количество сделок на одну симуляцию")
...
if st.button("▶️ Запустить симуляцию"):
    ...
    col1.metric("📌 Медиана", f"{np.median(balances):,.2f}", help="Серединное значение итоговых балансов (половина симуляций выше, половина — ниже)")
    col1.metric("🧠 Среднее", f"{np.mean(balances):,.2f}", help="Среднее арифметическое финальных балансов")
    col2.metric("🔻 Минимум", f"{np.min(balances):,.2f}", help="Худший итог среди всех симуляций")
    col2.metric("🟢 Максимум", f"{np.max(balances):,.2f}", help="Лучший итог среди всех симуляций")
    col3.metric("🎲 Стд. отклонение", f"{np.std(balances):,.2f}", help="Показатель разброса итогов — волатильность")
    col3.metric("📊 Симуляций", str(len(balances)))
    st.metric("💀 Ликвидации", f"{liq_count} из {len(balances)} ({liq_count / len(balances) * 100:.1f}%)", help="Сколько симуляций завершились обнулением баланса")
